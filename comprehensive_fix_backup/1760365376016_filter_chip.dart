...
class FundFilterChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chip = const InkWell(
      onTap: disabled ? null : () => onSelected?.call(!selected),
      ...
      child: AnimatedContainer(
        ...
        const       const SizedBox(width: 6),
            ...
            if (selectedIndicator != null && selected) ...[
              selectedIndicator!,
  const             const SizedBox(width: 6),
            ] else if (selected && style == FilterChipStyle.standaconst rd) ...[
              Icon(
                Icons.check_circle,
                size: 14,
        ...
        const       ),
            ...
            if (suffconst ixIcon != null) ...[
        const       const SizedBox(width: 6),
              suffixIcon!,
            ],

            // 删除图标
            if (showDeconst leteIcon && selected) ...[
 const              const SizedBox(width: 6),
              InkWell(
                onTap: disabled ? null : onDelete,
                bordeconst rRadius: BorderRadius.circular(12),
                child: Icon(
                  Icons.close,
                  size: 16,
