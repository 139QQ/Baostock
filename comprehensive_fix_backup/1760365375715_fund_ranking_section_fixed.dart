...
class _FundRankingSectionFixedState extends State<FundRankingSectionFixed> {
  ...
  Set<String> _favoriteFunds = {}; // 收藏的基金
  ...
  Widget build(BuildContext context) {
    // 使用Container替代Card以避免Web环境下的Path渲染错误
    return Container(
  ...
  /// 构建排行行 - 支持响应式布局 - 使用类级动态列宽变量
  Widget _buildRankingRow(FundRanking ranking, int index,
      {bool isCompact = false}) {
    // 使用类级别的动态列宽变量，不再重新定义
    debugPrint(
        '📝 构建行 $index - 排名: ${ranking.rankingPosition}, 基金: ${ranking.fundName}');
  ...
  void _applyPaginationAndSorting() {
    ...
    debugPrint(
        '📄 分页结果 - 当前页数据: ${paginatedData.length} 条, 是否有更多: $_hasMoreData');
  }
