...
class AppLogger {
  ...
  static void debug(String message, [dynamic data]) {
    if (enableDebugLogging && kDebugMode) {
      ...
      // ignore: avoid_print
      AppLogger.debug('🐛 DEBUG [$timestamp] $message ${data != null ? '- $data' : ''}');
    }
  }
  ...
  static void info(String message, [dynamic data]) {
    if (enableInfoLogging) {
      final timestamp = DateTime.now().toIso8601String();
      // ignore: avoid_print
      AppLogger.debug('ℹ️ INFO [$timestamp] $message ${data != null ? '- $data' : ''}');
    }
  }
  ...
  static void warn(String message, [dynamic data]) {
    if (enableWarnLogging) {
      final timestamp = DateTime.now().toIso8601String();
      // ignore: avoid_print
      AppLogger.debug('⚠️ WARN [$timestamp] $message ${data != null ? '- $data' : ''}');
    }
  }
  ...
  static void error(String message, dynamic error, [StackTrace? stackTrace]) {
    if (enableErrorLogging) {
      final timestamp = DateTime.now().toIso8601String();
      // ignore: avoid_print
      AppLogger.debug('❌ ERROR [$timestamp] $message - $error');
  ...
  static void network(
    ...
    if (enableDebugLogging && kDebugMode) {
      ...
      if (requestData != null) {
        // ignore: avoid_print
        AppLogger.debug('📤 Request: $requestData');
      }

      if (responseData != null) {
        // ignore: avoid_print
        AppLogger.debug('📥 Response: $responseData');
      }
    }
  }
  ...
  static void setLogLevel({
    ...
    AppLogger.debug('Logger configuration updated - debug: $enableDebugLogging, info: $enableInfoLogging, warn: $enableWarnLogging, error: $enableErrorLogging');
  }
