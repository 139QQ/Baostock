...
class SmartCacheManager {
  static const String _cacheBoxName = 'smart_fund_cache';
  int _maxMemoryCacheSize = 100; // 最大内存缓存条目数
  static const Duration _defaultTtl = Duration(hours: 1); // 默认缓存时间
  static const Duration _extendedTtl = Duration(hours: 6); // 扩展缓存时间
  ...
  Future<void> _persistToHive(String key, CacheEntry entry) async {
    try {
      if (_cacheBox is _InMemoryBox) {
        (_cacheBox as _InMemoryBox).put(key, entry);
        return;
      }
  ...
  void _cleanupExpiredCache() {
    ...
    for (final entry in _memoryCache.entries) {
      if (entry.value.isExpired ||
          now.difference(entry.value.createdAt) > _extendedTtl) {
    ...
    if (_cacheBox is! _InMemoryBox) {
      try {
        for (final key in _cacheBox.keys) {
          ...
          if (entry == null ||
              entry.isExpired ||
              now.difference(entry.createdAt) > _extendedTtl) {
