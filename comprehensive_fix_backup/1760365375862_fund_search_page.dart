...
import '../../../features/fund/presentation/widgets/filter_results.dart';
...
class _FundPageState extends State<FundSearchPage>
              ...
              if (state is FilterLoadSuccess) {
                return FilterPanel(
                  onFilterChanged: (criteria) {
                    ...
                    if (currentSearchState is SearchLoadSuccess) {
                      // 如果有搜索结果，可以重新搜索以应用筛选
                      final currentCriteria =
                          currentSearchState.searchResult.criteria;
                      final combinedCriteria = _combineSearchAndFilterCriteria(
                        currentCriteria,
                        criteria,
                      );
                      searchBloc.add(
                        PerformSearch(criteria: combinedCriteria),
                      );
                    }
                  },
                );
              } else if (state is FilterLoadFailure) {
                return Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
  ...
  /// 组合搜索和筛选条件
  FundSearchCriteria _combineSearchAndFilterCriteria(
    FundSearchCriteria searchCriteria,
    FundFilterCriteria filterCriteria,
  ) {
    // 如果筛选条件为空，返回原始搜索条件
    ...
    // 创建组合后的搜索条件
    // 这里将筛选条件转换为搜索条件的附加参数
    return searchCriteria.copyWith(
      // 可以添加筛选条件到搜索中
      // 例如：将筛选的关键词添加到搜索中
      searchFields: [
        SearchField.name,
      ...
      // 扩展搜索关键词以包含筛选条件
      extendedFilters: _convertFiltersToExtendedParams(filterCriteria),
    );
  }

  /// 将筛选条件转换为扩展搜索参数
  Map<String, dynamic> _convertFiltersToExtendedParams(
      FundFilterCriteria criteria) {
    final params = <String, dynamic>{};

    if (criteria.fundTypes?.isNotEmpty == true) {
      params['fundTypes'] = criteria.fundTypes!;
    }

    if (criteria.companies?.isNotEmpty == true) {
      params['companies'] = criteria.companies!;
    }

    if (criteria.scaleRange != null) {
      params['scaleMin'] = criteria.scaleRange!.min;
      params['scaleMax'] = criteria.scaleRange!.max;
    }
    ...
    if (criteria.returnRange != null) {
      params['returnMin'] = criteria.returnRange!.min;
      params['returnMax'] = criteria.returnRange!.max;
    }
