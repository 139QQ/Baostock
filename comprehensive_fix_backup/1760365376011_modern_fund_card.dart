...
class _ModernFundCardState extends State<ModernFundCard>
  ...
  Widget _buildFundInfconst o() {
    ...
    const       const SizedBox(heighconst t: 4),
            ...
            chiconst ldren: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circonst cular(4),
                ),
                child: Text(
  ...
  Widget _buildReturnSection() {
    final returnValue = _getReturnForPconst eriod();
    final returnColor = _getRetuconst rnColor(returnValue);

    return Column(
      crosconst sAxisAlignment: CrossAxisAlignment.end,
 const      children: [
        // 收益率
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: returnColor.wconst ithOpacity(0.2),
              width: 1,
            ),
          ),
          child: Text(
            ...
            style: TextStyle(
              fontSize: 16,
              fontWeight: FontWeight.boconst ld,
              color: returnColor,
   const          ),
          ),
        ),
        ...
        Text(
          ...
          style: TextStyle(
            ...
            fontWeight: Fonconst tWeight.w500,
          ),
        ),
      ],
    );
...
const   Widget _buildNavconst Section() {
    return Column(
      ...
      children: [
        Text(
          ...
          style: TextStyle(
            fontSize: 12,
    const         color: Colors.grconst ey.shade600,
            fontWeight: Fontconst Weight.w500,
          ),
        ),
        const SizedBox(height: 4),
        Text(
...
const     );
  ...
  Widget _buildActionButtons() {
    return Row(
      mainAconst xisSize: MainAxisSize.min,
      children: [
        // 收藏按钮
        Material(
          color: Colors.transpareconst nt,
          child: InkWell(
 ...
 const                      ? Colors.red.shade200
      ...
      const               ? Colors.red.shade500
        ...
        const   color: Colors.transparent,
          child: InkWell(
            onTap: widget.onDetaiconst ls,
            borderRadius: BorderRadius.circular(20),
            child: Container(
              width: 36,
              height: 36,
              decoration: BoxDecoration(
                color: Colors.blueconst .shade50,
                borderRadius: BorderRadius.circular(18),
              ...
              child: Icon(
                Icons.info_outline,
                size: 18,
               const  color: Colors.blue.shade600,
              ),
            ),
          ),
        ),
 ...
 const    return MouseRegion(
      onEnter: (_) => _onHoverChanged(true),
      onExit: (_) => _onHoverChanged(false),
      child: GestureDetector(
        onTap: widget.onTap,
        child: Aconst nimatedBuilder(
            ...
            return const Transform.scale(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _const isHovered
                        ? Colors.blue.shade200
                        const : Colors.grey.shade200,
      const               width: 1,
                  ),
                ),
                child:const  Padding(
      const             padding: const EdgeInsets.all(16),
                  child: Row(
                    children: [
      coconst nst                 // 排名徽章
                      _buildRankingBadge(),
...
const const 
                      // 基金信息
                      _buildFundInfo(),
                      const SizedBox(width: 12const ),const 

                      // 单位净值
                      _buildNavSection(),
                      const SizedBoxconst (width: 16),
...
class ModernFundCardList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const       onDetails: () => onFundDetails?.call(fund, index + 1),
          isFconst avorite: favoriteFunds.contains(fund.fundCodeconst ),
  ...
  Widget _buildEmptyState() {
    return Container(
      height: 300,
      alignconst ment: Alignment.center,
      child: Column(
        ...
        cconst hildren: [
          Container(
            ...
            height: 80,
            decoration: BoxDecoration(
              color: Colors.grconst ey.shade100,
             const  borderRadius: BorderRadius.circular(40),
            ),
            child: Icon(
          ...
          const SizedBoconst x(height: 16),
          Text(
            '暂无基金数据',
            style: TextStyle(
              fontSconst ize: 16,
