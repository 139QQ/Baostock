...
/// - 正负收益基金数量
/// - 更新时间
class RankingStatisticsWidget extends StatelessWidget {
  final RankingStatistics statistics;

  const RankingStatisticsWidget({
    super.key,
    ...
    return GridView.count(
      ...
      children: [
        ...
        _buildStatItem(
          context,
          '波动率',
          statistics.volatilityIndex.toStringAsFixed(2),
          Icons.show_chart,
          Colors.orange,
        ),
        _buildStatItem(
          context,
          '最大回撤',
          '${statistics.maxDrawdown.toStringAsFixed(2)}%',
          Icons.trending_down,
  ...
  Widget _buildReturnDistribution(BuildContext context) {
    ...
    final positiveRate = statistics.positiveReturnRate;
    final positiveCount = (total * positiveRate).round();
    final negativeCount = total - positiveCount;

    final positivePercentage = total > 0 ? positiveRate * 100 : 0;
    final negativePercentage = total > 0 ? (1 - positiveRate) * 100 : 0;
          ...
          Row(
            ...
            children: [
              _buildDistributionItem(
                '盈利基金',
                positiveCount,
                positivePercentage.toDouble(),
                const Color(0xFF10B981),
              ),
              _buildDistributionItem(
                '亏损基金',
                negativeCount,
                negativePercentage.toDouble(),
                const Color(0xFFEF4444),
              ),
            ],
          ),
