...
class _FundRankingWrapperSimpleState extends State<FundRankingWrapperSimple>
  ...
  void initState() {
    ...
    // 不在initState中初始化，在build方法中通过Builder和addPostFrameCallback初始�?
  }

  /// 初始化数据加�?- 需要在BlocProvider创建后调�?
  Future<void> _initializeData(BuildContext context) async {
    ...
    try {
      ...
      if (mounted && _cubit != null) {
        ...
        debugPrint('�?FundRankingWrapperSimple 初始化成�?);
      }
    } catch (e) {
      debugPrint('�?FundRankingWrapperSimple 初始化失�? $e');
      // 如果Provider仍然不可用，再重试一�?
      if (mounted && e.toString().contains('ProviderNotFoundException')) {
        ...
        try {
          ...
          if (mounted && _cubit != null) {
            _cubit!.initialize();
            _hasInitialized = true;
            debugPrint('�?FundRankingWrapperSimple 重试初始化成�?);
          }
        } catch (retryError) {
          debugPrint('�?FundRankingWrapperSimple 重试初始化失�? $retryError');
          _cubit = null;
        }
      }
    } finally {
  ...
  Future<void> _reloadData() async {
    ...
    if (!_hasInitialized && _cubit == null) {
      debugPrint('🔄 FundRankingWrapperSimple: 未初始化，先尝试初始�?);
  ...
  Future<void> _reloadWithFallback() async {
    debugPrint('🔄 FundRankingWrapperSimple: 重新获取Provider并重�?);
    try {
      _cubit = context.read<FundRankingCubit>();
      _cubit!.forceReload();
      debugPrint('�?FundRankingWrapperSimple 重新获取Provider并重载成�?);
    } catch (e) {
              ...
              return Card(
                ...
                child: Container(
                  ...
                  child: SingleChildScrollView(
                    child: Padding(
                      padding: const EdgeInsets.all(16), // 减少内边�?
                      child: Column(
  ...
  /// 构建底部错误操作按钮
  Widget _buildBottomErrorActions(BuildContext context, FundRankingState state) {
    return Container(
