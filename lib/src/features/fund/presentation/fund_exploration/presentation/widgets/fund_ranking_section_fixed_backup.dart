...
/// - 排行榜导出功�?/// - 基金详情快速查�?/// - 分页加载真实数据
/// - 现代化卡片视图和传统表格视图切换
class FundRankingSectionFixed extends StatefulWidget {
  final List<FundRanking>? fundRankings; // 外部传入的真实数�?  final bool isLoading; // 加载状�?  final VoidCallback? onLoadMore; // 加载更多回调
...
class _FundRankingSectionFixedState extends State<FundRankingSectionFixed> {
  ...
  Future<void> _loadRankings() async {
    // 使用外部加载状态，不设置内部状�?    if (widget.isLoading) return;

    // 如果有外部数据源，不应该使用模拟数据
    if (widget.fundRankings != null) {
      debugPrint('⚠️ 已有外部真实数据，跳过模拟数据加�?);
      return;
    }
  ...
  /// 检查数据质量问�?  bool _checkDataQualityIssues() {
    if (_localRankings.isEmpty) return false;

    // 检查是否有大量数据使用默认�?    int unknownTypeCount = 0;
  ...
  /// 获取基金类型的显示文�?  String _getFundTypeDisplay(String fundType) {
    return _getDisplayText(fundType, '--');
  }

  /// 获取基金公司的显示文�?  String _getCompanyDisplay(String company) {
    return _getDisplayText(company, '--');
  }

  /// 排序排行�?- 基于AKShare API实际字段
  void _sortRankings() {
    ...
    // 排序后重新计算排名位置（支持同分并列排名�?    _updateRankingsWithTies(_localRankings);
      ...
      builder: (context, constraints) {
        ...
        return Column(
          ...
          children: [
            ...
            // 控制按钮区域 - 增强响应式处�?            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  // 时间周期选择�?                  ConstrainedBox(
  ...
  /// 构建时间周期选择�?  Widget _buildPeriodSelector() {
    return SizedBox(
      height: 32,
  ...
  /// 构建基金类型选择�?  Widget _buildFundTypeSelector() {
    return Container(
      height: 32,
  ...
  /// 构建排序选择�?  Widget _buildSortSelector() {
    return Container(
      height: 32,
      padding: const EdgeInsets.symmetric(horizontal: 12),
      decoration: BoxDecoration(
  ...
  /// 构建排行榜说�?  String _getRankingDescription() {
  ...
  Widget _buildRankingTable() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (isCompact) {
          // 移动端紧凑布局 - 优先保证核心信息
          rankWidth = 32.0; // 排名 - 最小必要宽�?          codeWidth = 55.0; // 代码 - 紧凑显示
          nameWidth = 85.0; // 名称 - 核心信息，但减少宽度
          typeWidth = 35.0; // 类型 - 最小宽�?          navWidth = 50.0; // 净�?- 紧凑显示
          returnWidth = 55.0; // 收益�?- 紧凑显示
          actionWidth = 40.0; // 操作 - 最小按钮宽�?        } else {
          // 桌面端标准布局 - 平衡各列重要�?          rankWidth = 40.0; // 排名
          codeWidth = 70.0; // 代码
          nameWidth = 120.0; // 名称 - 关键信息，但不过度占用空�?          typeWidth = 50.0; // 类型
          navWidth = 65.0; // 净�?          returnWidth = 70.0; // 收益�?          actionWidth = 55.0; // 操作
        }

        // 只在桌面端且空间充足时进行比例放�?        if (!isCompact && availableWidth > 600) {
          ...
          if (calculatedTotalWidth < availableWidth) {
            final scaleFactor =
                math.min(availableWidth / calculatedTotalWidth, 1.3); // 最大放�?0%
        ...
        return Container(
          ...
          child: Column(
            children: [
              // 表头 - 使用横向滚动，修复边框样�?              SingleChildScrollView(
                scrollDirection: Axis.horizontal,
                child: Container(
                  padding:
              ...
              // 表格内容 - 修复：使用固定高度避免约束冲�?              SizedBox(
                height: math.min(_localRankings.length * 72.0 + 50,
                    400), // 限制最大高�?00px，每行约72px
  ...
  /// 构建排行�?- 支持响应式布局 - 使用类级动态列宽变�?  Widget _buildRankingRow(FundRanking ranking, int index,
      {bool isCompact = false}) {
    // 使用类级别的动态列宽变量，不再重新定义
    debugPrint(
        '📝 构建�?$index - 排名: ${ranking.rankingPosition}, 基金: ${ranking.fundName}');
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          // 基金名称 - 使用固定宽度避免约束冲突
          SizedBox(
            width: nameWidth, // 使用固定宽度避免Flexible在横向滚动中的约束冲�?            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  ranking.fundName,
          ...
          SizedBox(
            ...
            child: Container(
              padding: const EdgeInsets.symmetric(
                  horizontal: 4, vertical: 2), // 减少内边距避免换�?              decoration: BoxDecoration(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontSize: isCompact ? 10 : 11, // 紧凑模式下使用更小字�?                  color: _getFundTypeColor(ranking.fundType),
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: Text(isFiltered ? '重置筛�? : '重新加载'),
  ...
  /// 构建错误状态组�?  Widget _buildErrorWidget() {
    final isDataQualityIssue = widget.errorMessage?.contains('数据不完�?) == true ||
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              isDataQualityIssue ? '数据加载不完�? : '加载失败: ${widget.errorMessage}',
              style: TextStyle(
                color: isDataQualityIssue ? Colors.orange : Colors.red,
                fontWeight: FontWeight.w500,
              ),
  ...
  /// 构建加载状�?  Widget _buildLoadingWidget() {
    return Container(
      height: 200,
      alignment: Alignment.center,
      child: const Column(
      ...
      child: Row(
        ...
        children: [
            ...
            onPressed: _currentPage > 1
                ? () {
                    if (mounted) {
                      setState(() {
                        _currentPage--;
                        // 应用分页过滤和排�?                        _applyPaginationAndSorting();
                      });
                    }
                  }
                : null,
          ...
          Container(
            ...
            child: Text(
              '�?$_currentPage �?(�?${(_localRankings.length / _pageSize).ceil()} �?',
              style: const TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.w500,
                color: Color(0xFF1F2937),
  ...
  /// 应用分页过滤和排�?  void _applyPaginationAndSorting() {
    if (!mounted || widget.fundRankings == null) return;

    // 计算当前页的起始和结束索�?    final startIndex = (_currentPage - 1) * _pageSize;
    ...
    debugPrint('📄 分页应用 - 页码: $_currentPage, 开始索�? $startIndex, 结束索引: $endIndex');
    debugPrint('📄 分页结果 - 当前页数�? ${paginatedData.length} �? 是否有更�? $_hasMoreData');
  }
  ...
  void _updateRankingsWithTies(List<FundRanking> dataToSort) {
    ...
    // 获取当前排序键值（基于选中的时间段�?    double getSortValue(FundRanking ranking) {
      return _getReturnForPeriod(ranking);
    }

    // 重新计算排名，支持并�?    int currentRank = 1;
    int itemsInRank = 0;
    ...
    for (int i = 0; i < dataToSort.length; i++) {
      ...
      // 如果是第一个元素，或者值与上一个不同，则更新排�?      if (i == 0 || currentValue != previousValue) {
  ...
  /// 获取收益率颜�?  Color _getReturnColor(double returnValue) {
    ...
    return Row(
      ...
      children: [
        IconButton(
          ...
          tooltip: _viewMode == ViewMode.table ? '切换到卡片视�? : '切换到表格视�?,
        ),
      ],
    );
  }
  ...
  void _handleFundFavorite(FundRanking fund, int ranking) {
    setState(() {
      if (_favoriteFunds.contains(fund.fundCode)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('已取消收�?${fund.fundName}')),
        );
      } else {
        _favoriteFunds.add(fund.fundCode);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('已收�?${fund.fundName}')),
        );
      }
    });
  }
