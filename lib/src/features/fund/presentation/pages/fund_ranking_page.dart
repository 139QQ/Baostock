...
class _FundRankingPageState extends State<FundRankingPage>
  ...
  void _loadDefaultRanking() {
    context.read<FundRankingBloc>().add(const LoadFundRankings(
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      if (bloc.state.successData?.hasMoreData == true) {
        bloc.add(const LoadMoreRankings());
      }
    }
  }
      ...
      children: [
        ...
        BlocBuilder<FundRankingBloc, FundRankingState>(
          builder: (context, state) {
            ...
            if (statistics != null) {
              return RankingStatisticsWidget(statistics: statistics);
            }
  ...
  void _toggleSearch() {
    setState(() {
      ...
      if (!_showSearch) {
        ...
        // 清除搜索状态
        context.read<FundRankingBloc>().add(const ClearSearchRankings());
      }
    });
  }
  ...
  /// 处理基金收藏
  void _handleFundFavorite(String fundCode, bool isFavorite) {
    if (isFavorite) {
  ...
  void _handleMenuAction(String action) {
    switch (action) {
      case 'refresh':
        context.read<FundRankingBloc>().add(const RefreshFundRankings());
        break;
      case 'export':
        _showExportDialog();
        break;
