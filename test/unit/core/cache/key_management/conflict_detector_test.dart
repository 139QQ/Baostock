...
class CacheKeyConflictDetector {
  ...
  Future<List<CacheKeyConflict>> detectConflicts(List<String> cacheKeys) async {
    ...
    final keyInfoList = <MapEntry<String, CacheKeyInfo>>[];

    // 第一遍：解析所有有效键，保留重复
    for (final key in cacheKeys) {
      final info = _keyManager.parseKey(key);
      if (info != null) {
        keyInfoList.add(MapEntry(key, info));
      }
    }

    // 第二遍：检测冲突，包括重复键
    for (int i = 0; i < keyInfoList.length; i++) {
      for (int j = i + 1; j < keyInfoList.length; j++) {
        final key1 = keyInfoList[i].key;
        final info1 = keyInfoList[i].value;
        final key2 = keyInfoList[j].key;
        final info2 = keyInfoList[j].value;
  ...
  bool _isPotentialNamespaceConflict(CacheKeyInfo info1, CacheKeyInfo info2) {
    ...
    if (info1.type == info2.type) {
      ...
      // 跳过完全相同的标识符（这些会被其他规则处理）
      if (id1 == id2) {
        return false;
      }

      // 检查是否一个标识符是另一个的前缀（真正的包含关系）
      if ((id1.startsWith('${id2}_') || id2.startsWith('${id1}_')) &&
          (id1.length > 3 && id2.length > 3)) {
        // 确保标识符有一定长度
        return true;
      }

      // 检查层级关系（如 "fund_list" 和 "fund_list_equity"）
      final parts1 = id1.split('_');
      final parts2 = id2.split('_');

      // 如果一个标识符的所有部分都是另一个标识符的前缀部分
      if (parts1.length > 1 && parts2.length > 1) {
        final minLength = math.min(parts1.length, parts2.length);
        bool allMatch = true;
        ...
        if (allMatch && parts1.length != parts2.length) {
          return true;
        }
      }
    }
...
void main() {
  ...
  group('冲突解决建议测试', () {
    ...
    test('应该为重复冲突生成合适的建议', () {
      const conflict = CacheKeyConflict(
