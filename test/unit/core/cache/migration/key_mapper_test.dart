...
void main() {
  group('缓存键映射器基础功能测试', () {
    ...
    test('应该正确添加和管理映射规则', () {
      const rule1 = KeyMappingRule(pattern: 'old_', replacement: 'new_');
      const rule2 = KeyMappingRule(
    ...
    test('应该正确应用简单的字符串替换规则', () {
      mapper.addRule(const KeyMappingRule(
    ...
    test('应该识别已经是标准格式的键', () {
      const standardKey = 'jisu_fund_fundData_161725@latest';
    ...
    test('应该处理无法映射的键', () {
      const unmappableKey = 'completely_unknown_key_format';
  ...
  group('缓存键映射器批量操作测试', () {
    ...
    setUp(() {
      mapper = CacheKeyMapper();
      mapper.addRule(const KeyMappingRule(
          pattern: 'old_', replacement: 'jisu_fund_fundData_'));
    });
  ...
  group('缓存键映射器统计信息测试', () {
    ...
    test('应该提供正确的统计信息', () {
      ...
      // 添加规则后
      mapper.addRule(
          const KeyMappingRule(pattern: 'test', replacement: 'mapped'));
  ...
  group('KeyMappingRule对象测试', () {
    test('应该正确应用简单替换规则', () {
      const rule = KeyMappingRule(pattern: 'old', replacement: 'new');
